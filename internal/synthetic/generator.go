...
func buildSources(env contracts.Envelope) contracts.SourceCatalogue {
	sources := []contracts.Source{
		...
		{ID: "dut_control", Label: "DUT Control", NodeID: "reference_dut", ServedBy: "archive_node_a", Owner: "subsystem_test_role", Bus: "subsystem_event_bus", Quality: "fresh", FreshnessMS: 500, Provenance: "synthetic_fixture_generator", EvidenceSuitability: "supporting", Signals: []string{"obc_command_counter", "tc_packet_counter", "functional_gate_code"}},
		...
		{ID: "archive_quality", Label: "Source Quality Monitor", NodeID: "archive_node_a", ServedBy: "librarian_a", Owner: "test_conductor_role", Bus: "telemetry_bus", Quality: "synthetic", FreshnessMS: 1000, Provenance: "synthetic_fixture_generator", EvidenceSuitability: "supporting", Signals: []string{"source_freshness_ms", "facility_interlock_code"}},
		// NAS — long-term storage of completed tests as HDF5, plus legacy data (CSV, TXT, TDMS); all indexed by librarian_a
		{ID: "nas_exports", Label: "NAS Archival Store (HDF5 + legacy)", NodeID: "nas_a", ServedBy: "librarian_a", Owner: "test_conductor_role", Bus: "storage_bus", Quality: "fresh", FreshnessMS: 5000, Provenance: "synthetic_fixture_generator", EvidenceSuitability: "primary", Signals: []string{"overall_packet_counter", "tm_packet_counter", "dropped_frame_count"}},
		// Librarian — indexes NAS (HDF5, CSV, TXT, TDMS) and live Arrow IPC; any gateway can subscribe to translated streams on demand
		{ID: "librarian_index", Label: "Librarian (multi-format index & live translation)", NodeID: "librarian_a", ServedBy: "gateway_a", Owner: "test_conductor_role", Bus: "api_bus", Quality: "fresh", FreshnessMS: 2000, Provenance: "synthetic_fixture_generator", EvidenceSuitability: "supporting", Signals: []string{"overall_packet_counter", "bus_latency_ms", "source_freshness_ms"}},
		// Gateway — backend API; serves all translated and live data to web UI clients
		{ID: "gateway_status", Label: "Data Gateway Status", NodeID: "gateway_a", ServedBy: "gateway_a", Owner: "test_conductor_role", Bus: "api_bus", Quality: "fresh", FreshnessMS: 1000, Provenance: "synthetic_fixture_generator", EvidenceSuitability: "supporting", Signals: []string{"source_freshness_ms", "bus_latency_ms", "overall_packet_counter"}},
...
func buildCampaign(env contracts.Envelope, id string) contracts.Campaign {
	...
	case "integrated_system_fat":
		c.Name, c.Level, c.State, c.Result, c.Facility = "Integrated System FAT", "system", "complete", "fail", "thermal_chamber_a"
	default:
...
func buildSupervisorOverview(env contracts.Envelope, campaigns map[string]contracts.Campaign, telemetry map[string][]contracts.TelemetrySample) contracts.SupervisorOverview {
	lanes := []contracts.SupervisorLane{
		...
		supervisorLane("archive_capture", "Archive Evidence Capture", "archive_node_a", campaigns["integrated_system_fat"], "Transport capture and evidence packaging", "telemetry_bus", "REQ-DATA-QUALITY fail with disposition note", "degraded", []string{"Archive node receives virtualized transport events from the replay bus."}, telemetry["integrated_system_fat"], []heroSpec{{"bus_latency_ms", "Bus Latency", "virtual_bus_health", "ms", "archive_bus", 0, 250}, {"tm_packet_counter", "Packet counter", "telemetry_counter", "count", "archive_bus", 0, 8000}}),
	}
